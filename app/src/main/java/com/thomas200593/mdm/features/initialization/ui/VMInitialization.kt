...
class VMInitialization @Inject constructor(
    ...
    data class Form(
        val fldFirstName: CharSequence = STR_EMPTY, val fldFirstNameEnabled: Boolean = true, val fldFirstNameError: List<UiText> = emptyList(),
        val fldLastName: CharSequence = STR_EMPTY, val fldLastNameEnabled: Boolean = true, val fldLastNameError: List<UiText> = emptyList(),
        val fldEmail: CharSequence = STR_EMPTY, val fldEmailEnabled: Boolean = true, val fldEmailError: List<UiText> = emptyList(),
        val fldPassword: CharSequence = STR_EMPTY, val fldPasswordEnabled: Boolean = true, val fldPasswordError: List<UiText> = emptyList(),
        val btnProceedVisible: Boolean = false, val btnProceedEnabled: Boolean = true
    ) {
    ...
    private fun updateForm(transform: (Form) -> Form) {
        uiState.value.let { currentState ->
            ...
            if (updatedForm != state.scrData.form) { uiState.update { it.copy(scrDataState = state.copy(scrData = state.scrData.copy(form = updatedForm))) } }
        }
    }
    private fun updateDialog(transform: (DialogState) -> DialogState) =
        uiState.update { current -> current.copy(dialogState = transform(current.dialogState)) }
    private fun onProceedInitialization() {
        uiState.update { currentState ->
            ...
            if (!form.isAllFieldValid()) { return@update currentState.copy(resultInitialization = ResultInitialization.Error(Throwable("Invalid form fields"))) }
            currentState.copy(
