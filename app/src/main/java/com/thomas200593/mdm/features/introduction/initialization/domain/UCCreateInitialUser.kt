                    ...
                    when (error) {
                        is Error.Database.DaoQueryNoDataError -> when(dto.authType) {
                            is AuthType.LocalEmailPassword -> {
                                val input = dto.copy(
                                    authType = dto.authType.copy(
                                        password = bCrypt.hash(dto.authType.password)
                                    )
                                )
                                ...
                                val roles = input.toUserRoleEntity(
                                    uid = user.uid, roles = dto.initialSetOfRoles
                                )
                        ...
                        is Error.Database.DaoQueryError -> Result.failure(
                            Error.Database.DaoQueryError(
                                cause = error.cause
                            )
                        )
                        else -> Result.failure(
