...
interface DaoUser {
    ...
    fun getOneByUid(uid: String): Flow<UserEntity?>
...
class DaoUserImpl @Inject constructor(
    ...
    override fun getOneByUid(uid: String): Flow<UserEntity?> = appDatabase.daoUser().getOneByUid(uid).flowOn(ioDispatcher)
    override fun getOneByEmail(email: String): Flow<UserEntity?> = appDatabase.daoUser().getOneByEmail(email).flowOn(ioDispatcher)
}